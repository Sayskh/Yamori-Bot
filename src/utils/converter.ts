...
function runFfmpeg(args: string): Promise<void> {
    return new Promise((resolve, reject) => {
        exec(`"${FFMPEG}" ${args}`, { maxBuffer: 20 * 1024 * 1024, timeout: 30000 }, (error) => {
...
export async function imageToWebp(inputPath: string, outputPath: string): Promise<void> {
    await runFfmpeg(
        `-y -i "${inputPath}" -vf "scale=512:512:force_original_aspect_ratio=decrease,format=rgba,pad=512:512:(ow-iw)/2:(oh-ih)/2:color=#00000000,setsar=1" -vcodec libwebp -lossless 0 -compression_level 6 -q:v 60 -preset default -an -s 512:512 "${outputPath}"`
    );
}

export async function videoToWebp(inputPath: string, outputPath: string): Promise<void> {
    await runFfmpeg(
        `-y -i "${inputPath}" -t 5 -vf "fps=10,scale=512:512:force_original_aspect_ratio=decrease,format=rgba,pad=512:512:(ow-iw)/2:(oh-ih)/2:color=#00000000,setsar=1" -vcodec libwebp -lossless 0 -compression_level 6 -q:v 40 -loop 0 -preset default -an -fps_mode vfr -s 512:512 "${outputPath}"`
    );
}
...
export async function webpToVideo(inputPath: string, outputPath: string): Promise<void> {
    ...
    try {
        ...
        await runFfmpeg(
            `-y -f concat -safe 0 -i "${concatPath}" -vsync vfr -vf "scale=trunc(iw/2)*2:trunc(ih/2)*2,format=yuv420p" -an -c:v libx264 -crf 28 -preset faster -movflags +faststart "${outputPath}"`
        );
    } finally {
...
export async function mediaToMp4(inputPath: string, outputPath: string): Promise<void> {
    await runFfmpeg(
        `-y -i "${inputPath}" -vf "fps=20,scale=trunc(iw/2)*2:trunc(ih/2)*2,format=yuv420p" -an -c:v libx264 -crf 28 -preset faster -movflags +faststart "${outputPath}"`
    );
}
